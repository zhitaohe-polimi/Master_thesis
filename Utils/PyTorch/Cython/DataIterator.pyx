...
#     """
#
#     cdef int n_users, n_items, n_data_points, batch_size, n_sampled_points
#
...
#             raise StopIteration
#
#         # for i_batch in range(0, self.batch_size):#min(self.batch_size, self.n_data_points-self.n_sampled_points)):
...
cdef class InteractionIterator:
    ...
    def __init__(self, URM_train, positive_quota, batch_size = 1, set_fixed_len = None):
        super().__init__()

        URM_train = sps.coo_matrix(URM_train)
        self.n_users, self.n_items = URM_train.shape
        self.n_data_points = URM_train.nnz if set_fixed_len is None else set_fixed_len
        self.n_sampled_points = 0
...
cdef class BPRIterator:
    ...
    cdef int n_users, n_items, batch_size, n_data_points, n_sampled_points, n_negatives_per_positive
    ...
    def __init__(self, URM_train, batch_size = 1, set_fixed_len = None, n_negatives_per_positive = 1):
        ...
        self.n_users, self.n_items = URM_train.shape
        self.n_data_points = self.n_users if set_fixed_len is None else set_fixed_len
...
cdef class InteractionAndNegativeIterator:
    ...
    cdef int n_users, n_items, n_data_points, batch_size, n_sampled_points, n_negatives_per_positive
    ...
    def __next__(self):
        ...
        for i_batch in range(0, min(self.batch_size, self.n_data_points-self.n_sampled_points)):
            ...
            self.n_sampled_points +=1
            index = int(rand()*1.0/RAND_MAX*(self.n_data_points-1))
