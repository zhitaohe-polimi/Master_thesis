...
class _SimpleNewMFModel(torch.nn.Module):
    ...
    def forward(self, user, item):
        ...
        alpha_vi = torch.nn.functional.normalize(alpha_vi, dim=0)
        ...
        alpha_uj = torch.nn.functional.normalize(alpha_uj, dim=1)
...
class _PyTorchMFRecommender(BaseMatrixFactorizationRecommender, Incremental_Training_Early_Stopping):
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        if items_to_compute is not None:
            ...
            # alpha_vi = rescaling(alpha_vi, 0)
            alpha_vi = torch.nn.functional.normalize(alpha_vi, dim=0)
            summation_v = torch.einsum("bi,ib->bc", user_sim_uv, alpha_vi)
            item_scores += summation_v
