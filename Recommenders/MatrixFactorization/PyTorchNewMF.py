...
class _SimpleNewMFModel(torch.nn.Module):
    ...
    def forward(self, user, item):
        ...
        user_sim_uv = torch.corrcoef(self._embedding_user.weight.detach().cpu())[user]
        ...
        item_sim_ij = torch.corrcoef(self._embedding_item.weight.detach().cpu())[:, item]
...
class _PyTorchMFRecommender(BaseMatrixFactorizationRecommender, Incremental_Training_Early_Stopping):
    ...
    def fit(self, epochs=300,
        ...
        if self.RECOMMENDER_NAME == "PyTorchNewMF_BPR_Recommender_test":
...
class PyTorchNewMF_BPR_Recommender(_PyTorchMFRecommender):
    RECOMMENDER_NAME = "PyTorchNewMF_BPR_Recommender_test"
