...
from Recommenders.SLIM_BPR.BPR_sampling import BPR_Sampling
...
class SLIM_BPR_Python(BPR_Sampling, BaseItemSimilarityMatrixRecommender):
    ...
    def updateSimilarityMatrix(self):
        ...
        if self.topK != False:
            if self.sparse_weights:
                ...
                self.W = similarityMatrixTopK(self.S.T, k=self.topK)
    ...
    def epochIteration(self):
        ...
        for numCurrentBatch in range(totalNumberOfBatch):
            ...
            if(time.time() - start_time_batch >= 30 or numCurrentBatch==totalNumberOfBatch-1):
                print("Processed {} ({:4.1f}%) in {:.2f} seconds. Sample per second: {:.0f}".format(
