    ...
    try:
        ...
        if recommender_class is PyTorchNewMF_BPR_Recommender:
            hyperparameters_range_dictionary = {
                ...
                "batch_size": Categorical([16, 32, 64, 128, 256, 512, 1024]),
                "learning_rate": Real(low=1e-10, high=1e-8, prior='log-uniform'),
                "l2_reg": Real(low=1e-10, high=1e-8, prior='log-uniform'),
            }
        ...
        if recommender_class is PyTorchMF_MSE_Recommender:
            hyperparameters_range_dictionary = {
                ...
                "epochs": Categorical([1500]),
                "sgd_mode": Categorical(["sgd", "adagrad", "adam", "rmsprop"]),
                "batch_size": Categorical([16, 32, 64, 128, 256, 512, 1024]),  # 1, 2, 4, 8, 16, 32,
                "learning_rate": Real(low=1e-4, high=1e-1, prior='log-uniform'),
                "l2_reg": Real(low=1e-5, high=1e-2, prior='log-uniform'),
                "positive_quota": Real(low=0.0, high=0.5, prior='uniform'),
            }
